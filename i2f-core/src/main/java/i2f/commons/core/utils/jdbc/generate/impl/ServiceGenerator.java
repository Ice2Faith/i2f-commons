...
public class ServiceGenerator implements ITableGenerator {
    ...
    public String generate(GenerateContext ctx) {
        ...
        AppendUtil.AppendBuilder builder=AppendUtil.builder()
                ...
                .addsLine(" */")
                .addsLine("public interface I", ctx.castTableName(meta.getTableName()),"Service"," {");

        builder.addsLine("\tList<",ctx.castTableName(meta.getTableName()),"Bean> queryList(",ctx.castTableName(meta.getTableName()),"Bean bean);")
                .addsLine("\tboolean insert( List<",ctx.castTableName(meta.getTableName()),"Bean> list);")
                .addsLine("\tboolean update( ",ctx.castTableName(meta.getTableName()),"Bean bean);")
                .addsLine("\tboolean delete( ",ctx.castTableName(meta.getTableName()),"Bean bean);");
